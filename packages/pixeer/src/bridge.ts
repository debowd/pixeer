...
export function createPixeerBridge(
  ...
  const tracker: MutationTracker | null = options?.enableMutationTracker
    ? createMutationTracker(options.mutationTrackerOptions)
    : null;

  const deltaUnsubs: Array<() => void> = [];
  ...
  if (tracker) {
    ...
    transport.onMethod('dom.subscribe', async () => {
      if (!transport.notify) {
        return JSON.stringify({
          subscribed: false,
          reason: 'Transport does not support push notifications — use dom.getDelta to poll instead',
        });
      }

      const unsub = tracker.subscribe((deltas) => {
        transport.notify!('dom.delta', JSON.stringify({ deltas }));
      });
      deltaUnsubs.push(unsub);
  ...
  return {
    ...
    dispose() {
      for (const unsub of deltaUnsubs) unsub();
      tracker?.dispose();
